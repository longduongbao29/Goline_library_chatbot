...
class Agent:
    ...
    def search_book_info(self, state):
        ...
        if book:
            ...
            message = AIMessage(content="Xin lỗi, mình chưa tìm thấy được cuốn sách bạn muốn mua. Bạn có thể cung cấp thêm thông tin về tên sách, tác giả hoặc thể loại không?")
            return {"messages": [message]}
            
        logger.debug("checkpoint 1")
        search_term = state["order_info"].get("book_title", "")
        message = ToolMessage(
            content=f"Kết quả tìm kiếm cho '{search_term}': {book if book else 'Không tìm thấy sách phù hợp.'}",
            tool_call_id = "call_search_book"
        )
        state["messages"].append(message)
        return state
    def check_missing_info(self, state) :
        
        logger.debug(f"Order info: {state['order_info']}")
        ...
        search_fields = ["book_id", "author","category", "book_title_in_db"]
    ...
    def run(self, user_input, user_id):
        try:
            ...
            logger.error(f"Recursion error during agent run: {e}")
            return "Xin lỗi, mình chưa tìm thấy được cuốn sách bạn muốn mua. Bạn có thể "
